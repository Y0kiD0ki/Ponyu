...
#MUSIC
pygame.mixer.music.load('Music/8bit_Jojo.mp3')
...
boss_defeated_sound = pygame.mixer.Sound('Music/oof.wav')
...
class Boss(object):
    boss_sprite = [pygame.image.load('Sprites/Boss Sprites/Cinnabon Incinerator 1.png'), pygame.image.load('Sprites/Boss Sprites/Cinnabon Incinerator 2.png'), pygame.image.load('Sprites/Boss Sprites/Cinnabon Incinerator 3.png')]
    ...
    def __init__(self, x, y, width, height, end):
        ...
        self.vel = 3
        self.health = 100
        self.hitbox = (self.x + 30, self.y + 30, 200, 200)
        self.random_shot = random.randint(300,600)
        self.timecount = 0
        self.visible = True
    ...
    def draw(self, window):
        if self.visible == True:
            if self.walk_count + 1 >= 18:
                self.walk_count = 0
            if self.vel > 0:
                window.blit(self.boss_sprite[self.walk_count//6], (self.x, self.y))
                self.walk_count += 1
            pygame.draw.rect(window,(255,0,0),(self.hitbox[0],self.hitbox[1]-20, 50, 10))
            pygame.draw.rect(window,(0,169,0),(self.hitbox[0],self.hitbox[1]- 20, 50 - (5 * (10 - self.health)), 10))
...
start_button = Button((50, 206, 245), canvas_width/3, canvas_height/1.5, 250, 100, 'START')
...
show_boss_dialogue1 = Dialogue(560, 300, boss_dialogue1)
show_boss_dialogue2 = Dialogue(560, 300, boss_dialogue2)
...
while run:
    ...
    while start:
        ...
        window.blit(title_screen, (canvas_width/6, canvas_height/6))
    ...
    #Once Poñyu collects enough Cinnabons, the dialogue appears
    if cinnabon_delights >= 100:
        ...
        if boss.visible == True:
            if ponyu.hitbox[1] < boss.hitbox[1] + boss.hitbox[3] and ponyu.hitbox[1] + ponyu.hitbox[3] > boss.hitbox[1]:
                    if ponyu.hitbox[0] + ponyu.hitbox[2] > boss.hitbox[0] and ponyu.hitbox[0] < boss.hitbox[0] + boss.hitbox[2]:
                        ponyu.hit()
                        cinnabon_delights -= 5
        #Poñyu and boss collision
        for bullet in bullets:
            if bullet.y - 40 < boss.hitbox[1] + boss.hitbox[3] and bullet.y + 40 > boss.hitbox[1] and boss.visible == True:
                if bullet.x + 40 > boss.hitbox[0] and bullet.x +10 < boss.hitbox[0] + boss.hitbox[2]:
                    hit_sound.play()
                    boss.hit()
                    cinnabon_delights += 1
                    bullets.pop(bullets.index(bullet))
        ...
        #Transitions to the end screen after boss is defeated
        if boss.visible == False:
            pygame.mixer.music.pause()
            boss_defeated_sound.play()
            window.fill((0, 0, 0))
            end_screen = pygame.image.load('win.png')
            window.blit(end_screen, (canvas_width/5, canvas_height/5))
            pygame.display.update()
            pygame.time.delay(5000)
            break


    #Respawns enemies if 250 points have not been scored
    else:
        if cráneo_fuego.visible == False and cráneo_fuego_dos.visible == False:
            cráneo_fuego.y = random.randint(500,600)
            cráneo_fuego.vel += 1
